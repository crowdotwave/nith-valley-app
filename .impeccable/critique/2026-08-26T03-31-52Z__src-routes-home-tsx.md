...
**Contrast, checked manually** since the detector had no browser to measure in: `--muted #5f7590` on white is **4.74:1** — passes AA for normal text, but that is the tightest ratio on the page and `.tile-detail` renders it at 13px. Dark mode `--muted #93aabe` on `--surface #182333` is **6.58:1**, comfortable. `.summary-detail` at `opacity: .82` over navy is **8.85:1**. No contrast failures; the light-mode muted is the one to stop lowering.
